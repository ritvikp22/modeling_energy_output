...
atomicNum = {"H":1, "He": 2, "Be": 4, "C": 6, "N": 7, "O": 8, "F": 9, "Ne": 10, "Mg": 12, "Si": 14, "S": 16, "Ar": 18,
...
def memoizeComp(atomicNum, atomicMass, time):
    if(elements[species.get((atomicNum, atomicMass))][time] > -1):
        return elements[species.get((atomicNum, atomicMass))][time]
    last = memoizeComp(atomicNum, atomicMass, time-1)
    coef = dt*density[time-1] * atomicNum/m_h
